...
class GameField extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return GestureDetector(
      ...
      child: CustomPaint(
        size: screenSize,
        painter: GamePainter(gameState: gameState, screenSize: screenSize),
      ),
    );
  }
}
...
class GamePainter extends CustomPainter {
  ...
  void _drawPlane(Canvas canvas) {
    ...
    const planeY = 600; // фиксированное положение самолёта по вертикали

    // Корпус с эффектом размытия (след)
